...
interface UpdatePropertyTypeModalProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
}

export default function UpdatePropertyTypeModal({
    open,
    onOpenChange,
}: UpdatePropertyTypeModalProps) {
    const { form, onSubmit } = useCreatePropertyTypeVM();
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:!max-w-lg">
                <DialogHeader>Cập nhật loại hình bất động sản</DialogHeader>
                <Form {...form}>
