...
public class RedisChatMemoryRepositoryConfig {
  ...
  ObjectMapper createObjectMapper() {
    ...
    simpleModule.addDeserializer(
        ...
        new JsonDeserializer<>() {
          @Override
          public AssistantMessage deserialize(JsonParser p, DeserializationContext ctxt)
              throws IOException, JacksonException {
            p.nextToken();
            var content = p.getValueAsString();
            p.nextToken();
    ...
    simpleModule.addDeserializer(
        ...
        new JsonDeserializer<>() {
          @Override
          public SystemMessage deserialize(JsonParser p, DeserializationContext ctxt)
              throws IOException, JacksonException {
            p.nextToken();
            var content = p.getValueAsString();
            p.nextToken();

            return SystemMessage.builder().text(content).build();
          }
        });
    simpleModule.addDeserializer(
  ...
  static class MessageJsonSerializer extends JsonSerializer<Message> {
    ...
    public void serializeWithType(
        Message value, JsonGenerator g, SerializerProvider provider, TypeSerializer typeSer)
        throws IOException {
      WritableTypeId typeIdDef =
          typeSer.writeTypePrefix(g, typeSer.typeId(value, JsonToken.START_OBJECT));
